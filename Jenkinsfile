    ...
    stages {
        ...
        stage('check') {
            steps {
                ...
                secretKeyVariable: 'AWS_SECRET_ACCESS_KEY']]){
                    script {
                        def IStatus = sh(returnStdout: true, script: 'aws ec2 describe-instance-status --instance-ids i-0335eeb394f10ee2d --region eu-central-1 --query "InstanceStatuses[0].InstanceStatus.Status" --output text', returnStatus: true)
